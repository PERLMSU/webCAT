...
def get_rotation_groups_by_instructor_home(instructor, classroom):
    ...
    return RotationGroup.objects.filter(rotation__start_week__lte=week, rotation__end_week__gte=week,
                                        instructor=instructor, rotation__semester=classroom.current_semester,
                                        rotation__classroom=classroom).order_by('group_number')
    ...
    if form.is_valid():
        ...
        try:
            ...
            messages.add_message(request, messages.ERROR,
                                 "An error occurred when attempting to register the class: " + str(e))
    ...
    def post(self, *args, **kwargs):
        try:
            ...
            messages.add_message(self.request, messages.ERROR,
                                 'Unable to delete students from this classroom %s' % classroom)
        finally:
        ...
        if form.is_valid():
            ...
            if not checked_groups:
                messages.add_message(self.request, messages.SUCCESS,
                                     'Successfully unassigned all groups from this instructor.')
            ...
            for group_pk in checked_groups:
                ...
                try:
                    ...
                    group.save()
                    messages.add_message(self.request, messages.SUCCESS, 'Successfully assigned group ' + str(
                        group.group_number) + ' to %s' % instructor.get_full_name())
                except Exception as e:
                    messages.add_message(self.request, messages.ERROR,
                                         'Unable to assign group numbers to this instructor %s' % e)
            return HttpResponseRedirect('/classroom/')
        else:
            messages.error(self.request, form.errors)
            return HttpResponseRedirect('/classroom/')
                ...
                try:
                    ...
                    if count != MAX_STUDENTS:
                        ...
                        messages.add_message(self.request, messages.WARNING,
                                             'Could not add ' + student.first_name + ' ' + student.last_name + ' to group - already 4 members')
                except Exception as e:
                    messages.add_message(self.request, messages.ERROR,
                                         'Unable to assign student ' + student.first_name + ' ' + student.last_name + ' %s' % e)

                count = group.students.count()

            if successfully_added:
                messages.add_message(self.request, messages.SUCCESS,
                                     'Successfully added ' + ", ".join(successfully_added) + ' to group.')
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            try:
                ...
                messages.add_message(self.request, messages.ERROR,
                                     'Unable to assign this group to this instructor %s' % e)
            return HttpResponseRedirect('/classroom/')
        else:
            messages.error(self.request, form.errors)
            return HttpResponseRedirect('/classroom/')
...
class UploadStudentsView(LoginRequiredMixin, SuperuserRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if classroom != None:
            ...
            students = Student.objects.filter(classroom=classroom, semester=classroom.current_semester).order_by(
                'last_name')
        ...
        if form.is_valid():
            ...
            if self.email_already_exist:
                self.add_message(str(
                    self.email_already_exist) + " Students were not added because their email address already exists.")
...
class ClassroomView(LoginRequiredMixin, SuperuserRequiredMixin, TemplateView):
    ...
    def get(self, *args, **kwargs):
        ...
        if classroom != None:
            ...
            rotation_groups = RotationGroup.objects.filter(rotation__semester=classroom.current_semester,
                                                           rotation__classroom=classroom).order_by('group_number')
            ...
            assign_instructor_form.fields['instructor_id'].choices = [(la[0], la[1] + ' ' + la[2]) for la in
                                                                      learning_assistants.values_list('id',
                                                                                                      'first_name',
                                                                                                      'last_name').order_by(
                                                                          'last_name')]
            ...
            assign_groups_form.fields['group_numbers'].choices = rotation_groups.values_list('id',
