...
class AddEditRotationForm(forms.ModelForm):
    ...
    def clean_start_week(self):
        ...
        for rotation in rotations:
            if int(self.cleaned_data['start_week']) < rotation.start_week + rotation.length and int(
                    self.cleaned_data['start_week']) >= rotation.start_week:
    ...
    def clean_end_week(self):
        ...
        for rotation in rotations:
            if int(self.data.get('end_week')) > rotation.start_week and int(
                    self.data.get('start_week')) < rotation.start_week:
...
class EditClassroomForm(forms.Form):
    ...
    def clean_current_week(self):
        ...
        if (self.cleaned_data['current_week']) < 1 or (int(self.cleaned_data['current_week'])) > int(
                semester.get_number_weeks()):
...
class AssignMultipleGroupsForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['group_numbers'].choices = tuple(
            RotationGroup.objects.all().values_list('id', 'group_number').order_by('group_number'))
