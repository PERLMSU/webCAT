defmodule WebCATWeb.ExplanationControllerTest do
  ...
  describe "index/3" do
    test "responds normally to a well formed request", %{conn: conn} do
      ...
      result =
        conn
        |> Auth.sign_in(user)
        |> get(Routes.explanation_path(conn, :index, filter: %{feedback_id: feedback.id}))
        |> json_response(:ok)
  ...
  describe "delete/3" do
    test "responds normally to a well formed request", %{conn: conn} do
      {:ok, user} = login_admin()
      data = Factory.insert(:explanation)

      conn
      |> Auth.sign_in(user)
      |> delete(Routes.explanation_path(conn, :delete, data.id))
      |> response(:no_content)

      conn
      |> Auth.sign_in(user)
      |> get(Routes.explanation_path(conn, :show, data.id))
