...
type EditorState
    = StudentSelection (APIData (List RotationGroup))
    | ViewStudent (APIData (List RotationGroup)) RotationGroup User (APIData (List Category))
...
init session =
    case Session.credential session of
        Just cred ->
            ...
            , Users.rotationGroups session (.id user) GotRotationGroups
            )
