        ...
        if classroom != None:
            if 'week' in self.kwargs:
                ...
                if request.user.current_classroom.current_week:
                    ...
                    week = 1
            self.context['draft_notifications_need_approval'] = Notification.objects.filter(user=self.request.user,
                                                                                            draft_to_approve__status=1,
                                                                                            draft_to_approve__week_num=week)
            self.context['draft_notifications_need_revision'] = Notification.objects.filter(user=self.request.user,
                                                                                            draft_to_approve__status=2,
                                                                                            draft_to_approve__week_num=week)
            self.context['draft_notifications_approved'] = Notification.objects.filter(user=self.request.user,
                                                                                       draft_to_approve__status=3,
                                                                                       draft_to_approve__week_num=week)

            self.context['drafts_need_approval'] = Draft.objects.filter(week_num=week, student__classroom=classroom,
                                                                        student__semester=classroom.current_semester,
                                                                        status=1)
            self.context['drafts_need_revision'] = Draft.objects.filter(week_num=week, student__classroom=classroom,
                                                                        student__semester=classroom.current_semester,
                                                                        status=2)
            self.context['drafts_approved'] = Draft.objects.filter(week_num=week, student__classroom=classroom,
                                                                   student__semester=classroom.current_semester,
                                                                   status=3, email_sent=False)
            self.context['drafts_emailed'] = Draft.objects.filter(week_num=week, student__classroom=classroom,
                                                                  student__semester=classroom.current_semester,
                                                                  status=3, email_sent=True)
...
class ApproveAllDrafts(SuperuserRequiredMixin, LoginRequiredMixin, TemplateView):
    ...
    def get(self, *args, **kwargs):
        ...
        drafts = Draft.objects.filter(student__classroom=classroom, student__semester=classroom.current_semester,
                                      week_num=week, status=1)
        for draft in drafts:
            draft.status = 3
            draft.save()
...
class ApproveDraft(SuperuserRequiredMixin, LoginRequiredMixin, TemplateView):
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            grades_values = dict(
                [(name[6:], value) for name, value in self.request.POST.items() if name.startswith('grade_')])
        ...
        for i in range(len(selected_drafts)):
            ...
            try:
                ...
                messages.add_message(self.request, messages.ERROR,
                                     "Something went wrong. A draft could not be approved.")
...
class SendSelectedDrafts(SuperuserRequiredMixin, LoginRequiredMixin, TemplateView):
    ...
    def post(self, *args, **kwargs):
        ...
        if status == 3:
            ...
            selected_drafts = [value for name, value in self.request.POST.items() if
                               name.startswith('draft_to_resend_')]
        ...
        if successfully_sent:
            messages.add_message(self.request, messages.SUCCESS,
                                 str(len(successfully_sent)) + ' Feedback Emails have been sent to: ' + ', '.join(
                                     successfully_sent))
        if no_email_count:
            messages.add_message(self.request, messages.ERROR, str(
                len(no_email_found)) + ' Emails could not be sent, no email addresses found for: ' + ', '.join(
                no_email_found))
...
class SendDrafts(SuperuserRequiredMixin, View):
    ...
    def get(self, *args, **kwargs):
        ...
        sent_count = 0
        drafts = Draft.objects.filter(student__classroom=self.request.user.current_classroom,
                                      student__semester=self.request.user.current_classroom.current_semester,
